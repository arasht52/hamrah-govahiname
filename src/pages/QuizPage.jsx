        ...
        {q.opts.map((opt, i) => {
          ...
          if (!answered) {
            if (isSelected) {
              ...
              ltrBg = "#FF9500"; ltrColor = "#0A2540";
            }
          } else if (!examMode) {
            // حالت تمرین — نشون بده درست/غلط
            ...
            // حالت آزمون — فقط انتخاب کاربر خاکستری
            if (isSelected) { bg = "#112240"; border = "#4D6080"; }
          }

          return (
            <button key={`opt-${q.id}-${i}`} onClick={() => toggleOption(i)} disabled={answered}
              style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", textAlign: "right", padding: "14px 16px", borderRadius: 12, border: `2px solid ${border}`, background: bg, color, cursor: answered ? "default" : "pointer", fontFamily: "inherit", fontSize: 14, fontWeight: 600, transition: "all .15s" }}>
